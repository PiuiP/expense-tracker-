...
class TransactionService:
    ...
    # TODO: в абстрактный класс репозитория добавить метод,
    # который будет на sql сортировать по типу операции
    # тогда этот сервис будет просто вызывать метод репозитория
    # и возвращать результат -> rout-> client
    async def get_transaction_stats(self) -> dict:
        ...
        for t in transaction:
            amount = (
                ...
                if not isinstance(t.amount, Decimal)
                else t.amount
            )
