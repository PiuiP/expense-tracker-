...
async def create_category_handler(request: web.Request) -> web.Response:
    ...
    try:
        ...
        return web.json_response(data=result.model_dump(mode="json"), status=201)
...
async def get_category_handler(request: web.Request) -> web.Response:
    ...
    try:
        ...
        return web.json_response(data={"error": str(e)}, status=404)
...
async def delete_category_handler(request: web.Request) -> web.Response:
    ...
    try:
        ...
        if deleted:
            return web.json_response(
                data={"message": "Category deleted successfully"}, status=200
            )
        else:
            return web.json_response(data={"error": "Category not found"}, status=404)
    except Exception as e:
        return web.json_response(data={"error": str(e)}, status=500)
