...
async def create_transaction_handler(request: web.Request) -> web.Response:
    ...
    try:
        ...
        result: TransactionResponse = await service.create_transaction(
            transaction_data, category_ids
        )
...
async def get_all_transactions_handler(request: web.Request) -> web.Response:
    ...
    try:
        ...
        return web.json_response(
            data=[t.model_dump(mode="json") for t in result], status=200
        )
...
async def filter_transactions_handler(request: web.Request) -> web.Response:
    ...
    try:
        ...
        if not date_from or not date_to:
            return web.json_response(
                data={"error": "date_from and date_to are required"}, status=400
            )
        ...
        )  # if UUID(None) -> ERRRROR((((
        result = await service.get_filtered_transactions(cat_uuid, date_from, date_to)

        # result - list -> model_dump for each element
        # (!!!!!model_dump cannot be applied to lists, only pydentic-models!!!!!)
        return web.json_response(
            data=[t.model_dump(mode="json") for t in result], status=200
        )
...
async def get_expenses_by_category_handler(request: web.Request) -> web.Response:
    ...
    try:
        ...
        return web.json_response(data=expenses, status=200)
    except Exception as e:
        return web.json_response(data={"error": str(e)}, status=500)
...
async def get_transaction_handler(request: web.Request) -> web.Response:
    ...
    try:
        ...
        return web.json_response(data={"error": "Invalid UUID format"}, status=400)
    ...
    try:
        ...
        if result is None:
            return web.json_response(
                data={"error": "Transaction not found"}, status=404
            )

        return web.json_response(data=result.model_dump(mode="json"), status=200)
...
async def delete_transaction_handler(request: web.Request) -> web.Response:
    ...
    try:
        ...
        return web.json_response(
            data={"message": "Transaction is deleted successfully"}, status=200
        )
