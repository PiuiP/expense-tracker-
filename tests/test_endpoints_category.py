...
def category_payload():
    return {"name": "Groceries", "description": "Food and drinks"}


@pytest.fixture
def category_payload_2():
    return {"name": "Transport", "description": "Bus and taxi"}


@pytest.fixture
def invalid_payload_no_name():
    return {"description": "Test without name"}


@pytest.fixture
def invalid_payload_wrong_type():
    return {"name": 123, "description": "Test with wrong type"}
...
async def test_update_category_invalid_uuid(app):
    async with TestClient(TestServer(app)) as client:
        async with client.put(
            "/categories/invalid-uuid", json={"description": "Test"}
        ) as resp:
            assert resp.status == 400
