...
async def test_create_transaction_invalid_wrong_type(app, invalid_payload_wrong_type):
    async with TestClient(TestServer(app)) as client:
        async with client.post(
            "/transactions", json=invalid_payload_wrong_type
        ) as resp:
            assert resp.status == 400
...
async def test_get_by_id_success(app, transaction_payload):
    async with TestClient(TestServer(app)) as client:
        async with client.post(
            "/transactions", json=transaction_payload
        ) as create_resp:
...
async def test_get_all_success(app, transaction_payload, transaction_payload_income):
    async with TestClient(TestServer(app)) as client:
        ...
        async with client.post(
            "/transactions", json=transaction_payload_income
        ) as resp2:
...
async def test_update_transaction_success(app, transaction_payload):
    async with TestClient(TestServer(app)) as client:
        ...
        update_data = {"description": "Обновлённое описание", "amount": 600.00}

        async with client.put(
            f"/transactions/{transaction_id}", json=update_data
        ) as update_resp:
...
async def test_update_transaction_invalid_uuid(app):
    async with TestClient(TestServer(app)) as client:
        async with client.put(
            "/transactions/invalid-uuid", json={"description": "Тест"}
        ) as resp:
            assert resp.status == 400
...
@pytest.mark.asyncio
async def test_filter_transactions_success(
    app, transaction_payload, transaction_payload_income
):
    async with TestClient(TestServer(app)) as client:
        async with client.post("/transactions", json=transaction_payload) as resp1:
            assert resp1.status == 201
        ...
        async with client.get(
            "/transactions/filter?date_from=2026-09-01&date_to=2026-09-30"
        ) as filter_resp:
